...
export const API_BASE_URL =
  import.meta.env.VITE_API_URL || "http://localhost:8080"; // A URL base da sua API Java
...
async function authenticatedFetch(
  ...
  const token = CryptoJS.AES.decrypt(encryptedToken, ENCRYPTION_KEY).toString(
    CryptoJS.enc.Utf8
  );
...
export const usersApi = {
  ...
  updateUser: async (
    id: number,
    userData: Partial<Pick<Usuario, "nome" | "email" | "perfil">>
  ): Promise<Usuario> => {
    const response = await authenticatedFetch(`/usuarios/${id}`, {
      method: "PUT",
      body: JSON.stringify(userData),
